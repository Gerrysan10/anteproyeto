...
class ProgresocargasController {
    ...
    static async indexPost(req, res) {
        try {
            ...
            const insertedId = await progresocargasModel.insertar(newData);

            res.status(201)
                .header('Location', `/progresocargas/${insertedId}`)
    ...
    static async itemPatch(req, res) {
        try {
            ...
            const result = await progresocargasModel.actualizar(id, updatedFields);
